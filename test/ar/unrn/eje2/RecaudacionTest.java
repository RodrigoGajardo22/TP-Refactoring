...
public class RecaudacionTest {
	...
	public void testWhereGivenCompany() throws IOException {
		...
		Assert.assertEquals(new Recaudacion(new CSV()).where(options).size(), 7);
	}
	...
	public void testWhereGivenCity() throws IOException {
		...
		Assert.assertEquals(new Recaudacion(new CSV()).where(options).size(), 3);
	}
	...
	public void testWhereGivenState() throws IOException {
		...
		Assert.assertEquals(new Recaudacion(new CSV()).where(options).size(), 873);
	}
	...
	public void testWhereGivenRound() {
		try {
			Map<String, String> options = new HashMap<String, String>();
			options.put("round", "a");
			Assert.assertEquals(new Recaudacion(new CSV()).where(options).size(), 582);
		} catch (IOException e) {
		}
	}
	...
	public void testMultipleOptions() throws IOException {
		...
		options.put("round", "a");
		options.put("company_name", "Facebook");
		Assert.assertEquals(new Recaudacion(new CSV()).where(options).size(), 1);
	}
	...
	public void testWhereNotExists() throws IOException {
		...
		Assert.assertEquals(new Recaudacion(new CSV()).where(options).size(), 0);
	}
	...
	public void testWhereCorrectKeys() throws IOException {
		Map<String, String> options = new HashMap<String, String>();
		options.put("company_name", "Facebook");
		Map<String, String> row = new Recaudacion(new CSV()).where(options).get(0);
